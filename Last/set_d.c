...
/*   Updated: 2016/03/08 13:25:10 by jle-quer         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char		*set_str(t_struct *form, char *str, long long n)
{
	if (n == 0 && form->prec == 0)
		...
		str = set_d_0(ft_itoa_base(n, 10), form->prec - ft_count(n), n);
	else if (form->prec == ft_count(n) && n < 0)
		str = set_d_0_neg(ft_itoa_base(-n, 10), form->prec - ft_count(n), -n);
	else
		str = ft_itoa_base(n, 10);
	return (str);
}
